...
export class KycController {
  ...
  async upload(
    @Headers('authorization') authHeader: string,
    @Body() body: { tenantId: string; filename: string; fileBase64: string },
  ) {
    const user = await getUserFromAuthHeader(this.supabaseService.getClient(), authHeader);
    if (!body.tenantId || !body.filename || !body.fileBase64) throw new BadRequestException('tenantId, filename and fileBase64 are required');
