...
**Last aligned with codebase:** `main` @ June 2026 (`4871ee5`)
...
| Roadmap item | Today on `main` | Gap |
|--------------|-----------------|-----|
| **1.1 Named tiers** | `BUILDING` / `FAIR` / `GOOD` / `EXCELLENT` + `risk_tier` | Rebrand to Bronze→Platinum narrative, tier copy, "next tier" UX, progress meter |
| **1.1 Score explainer** | Factor weights on `/tenant/credit-score` | No actionable "holding you back" engine or point deltas |
| **1.1 Simulator** | None | New API projection + UI slider |
| **1.3 Shareable PDF** | Server PDF via `/reports/*`; basic tenant reports page | Redesign, tier badge, QR, expiry, share links |
| **1.3 Verification URL** | `/verify/[reference]` + `GET /reports/verify/:ref` | Extend payload (tier, history hash, expiry); custom domain |
| **1.2 Auto-confirm** | Landlord manual confirm only | Cron job + `auto_confirm_at` + dispute window logic |
| **1.2 SMS/WhatsApp nudge** | Email only; `NEXT_PUBLIC_SMS_ENABLED=false` | SMS provider integration + templates |
| **1.2 One-tap confirm link** | None | Signed JWT action links (`/confirm-payment/[token]`) |
| **1.2 Confirmation dashboard** | Payments list in landlord UI | Dedicated pending/aging view + stats |
| **1.4 Dispute timeline** | Dispute APIs + admin UI | Tenant/landlord timeline component + realtime optional |
| **1.4 Evidence templates** | Free-form file upload | Typed dispute flows + checklist migrations |
| **1.4 Outcome analytics** | Admin audit partial | `dispute_outcomes` / analytics rollup table + admin charts |
| **1.5 POPIA badge** | Counsel-review banner on legal pages | In-app wizard badge + plain-language summary page |
| **1.5 NA bank refs** | Generic EFT proof upload | Reference parser/validator per bank |
| **1.5 Suburb names** | Suburb in DB / MI | Ensure Windhoek suburbs surface in tenant + marketing UI |

**Phase 2+:** largely net-new (waitlist, BYOL, CSV bulk units, public `data.crenit.na`, sample API keys). B2B MI admin exists; public layer and self-serve demo do not.
...
- [ ] Median landlord confirmation lag &lt; 48h (auto-confirm + nudges).
- [ ] Tenant NPS proxy: &gt;60% generate or share a PDF within 90 days of KYC approve.
- [ ] Score page session time up; support tickets on "what is my score" down.
- [ ] Dispute "where is my case" contacts down vs baseline.
- [ ] Flywheel metrics dashboard (internal): confirm lag, score refresh rate, MI capture rate.
...
*Phase 1 is the engineering priority. No Phase 2 feature work starts until Phase 1 exit criteria are met or explicitly waived by product.*
